...
// A group of shapes, defined as a unit
// Operations apply to all shapes
// This is kind of weird - some disconnects between 
// what it should do and it what it actually does 
// (e.g., you can change the color of a shape in a group
// by changing the color of the original variable!)
// Really, this shouldn't inherit from shape (it's not really a shape)
// It should be it's own class and we should modify window to be able
// to call the draw function on this class along with for shape
// That will fix some of the disconnects (you can set the color
// as you draw, since we don't have to set draw to be const) 
// (you don't have to delete point and number_of_points)
// Also this is bad, doesn't use good object oriented programming stuff
// (e.g., it redefines fcolor and lcolor)
// Look at binary tree to see a much better example
struct Group : Shape {
public:
	void add(Shape& s);
	...
	Point point(int i) const = delete;
	int number_of_points() const = delete;
protected:
...
struct Function : Shape {
	...
	Function(Fct f, double r1, double r2, Point orig, 
			int count = 100, double xscale = 25, 
			double yscale = 25);
};
...
struct Striped_rectangle : Rectangle {
	Striped_rectangle(Point xy, int w, int h) :
				Rectangle{xy, w, h} { }
...
// Polygon that doesn't allow adding points after construction
struct Poly : Shape {
	Poly(initializer_list<Point> ps);
protected:
	void draw_lines() const;
	Poly() {} // So derived classes can create their own constructor
};
...
struct Arc : Shape {
	Arc(Point p, int rr, double d1, double d2) :
			r{rr}, deg1{d1}, deg2{d2}
	{
		add(Point{p.x-r, p.y-r});
	}
	void draw_lines() const;
...
struct Right_triangle : Poly {
	Right_triangle(Point p1, Point p2, Point p3);
};

struct Hexagon : Poly {
	Hexagon(Point c, int h, int w);
};
...
public:
	Binary_tree(Point origin, int levels, int tree_width = 500,
		int line_height = 60, int node_width = 16, 
		int node_height = 16);
	void set_label(int i, string label);
	string label(int i) const;

protected:
	void draw_lines() const override;
	// See what happens when you remove the "virtual" here!
	// A shape object that is a Binary_tree_sqr won't know
	// that draw_node is a virtual function when it's called
	// in draw_lines, so the usual "circle" draw node is called, 
	// drawing circles rather than squares.
	virtual void draw_node(Point n) const;
	virtual void draw_edge(Point c, Point p) const;
...
private:
	...
	vector<string> lbls;
	const int fnt = fl_font();
	static constexpr int fnt_sz = 14;
protected:
	int n_width;
	int n_height;
};

struct Binary_tree_sqr : Binary_tree {
	Binary_tree_sqr(Point origin, int levels, 
		int tree_width = 500, int line_height = 60, 
		int node_width = 16, int node_height = 16);
protected:
	void draw_node(Point n) const override;
};

struct Bar_graph : Shape {
	Bar_graph(Point origin, int xlength, int ylength, 
				int yscale, int yoffset = 1, string name = "");
	void add(double val, string label = "") {
		if (val < 0) {
			error("Bar_graph::add: "
				"Attempted to add negative value");
		}
		vals.push_back(val);
...
struct Clock : Shape {
	Clock(Point center = Point{0,0}, int radius = 100);
protected:
	void draw_lines() const override;
private:
	int r;
