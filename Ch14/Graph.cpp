...
void Binary_tree::draw_label(Point n, string lbl) const {
	if (color().visibility()) { // Ensure proper color 
		fl_color(color().as_int()); // before drawing
	}
	fl_font(fnt, fnt_sz);
	fl_draw(lbl.c_str(), n.x - n_width/2 + 1, n.y + n_height/4);
